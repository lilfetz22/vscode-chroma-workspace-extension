...
// Map common CSS color names to hex and build reverse lookup for tooltips
...
const HEX_TO_NAME: Record<string, string> = Object.keys(CSS_COLOR_MAP).reduce((acc, key) => {
...
function hexToName(hex: string): string | undefined {
    if (!hex) return undefined;
    const normalized = hex.trim().toUpperCase();
    return HEX_TO_NAME[normalized]?.charAt(0).toUpperCase() + HEX_TO_NAME[normalized]?.slice(1);
}
